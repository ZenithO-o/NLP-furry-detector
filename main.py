...
class TwitterAuthenticator():
    ...
    def authenticateTwitterApp(self):
        ...
        if (len(twitterkey["consumer_key"]) < 20) or (len(twitterkey["consumer_secret"]) < 30) or (len(twitterkey["access_token"]) < 30) or (len(twitterkey["access_token_secret"]) < 30):
            self.has_key = False
...
class TwitterClient():
    ...
    def __init__(self, twitter_user=None):
        ...
        self.twitter_client = API(self.authenticator.authenticateTwitterApp(), wait_on_rate_limit=True)
...
FIELD_BG = "snow"
COLOR_GRADIENT = ['#f00', '#fc0a00', '#fa1400', '#f71e00', '#f52800', '#f23100', '#ef3b00', '#ed4400', 
                '#ea4c00', '#e85500', '#e55e00', '#e26600', '#e06e00', '#dd7500', '#db7d00', '#d88400', 
                '#d68b00', '#d39200', '#d09900', '#cea000', '#cba600', '#c9ac00', '#c6b200', '#c3b700', 
                '#c1bd00', '#babe00', '#b0bc00', '#a6b900', '#9cb600', '#93b400', '#89b100', '#80af00',
                '#77ac00', '#6fa900', '#66a700', '#5ea400', '#56a200', '#4e9f00', '#469d00', '#3f9a00', 
                '#389700', '#319500', '#2a9200', '#239000', '#1d8d00', '#178a00', '#180', '#0b8500', 
                '#058300', '#008000']
...
def testUser():
    ...
    if (input_user != "Input Twitter user here... ex: @zenithO_o"):
        ...
        try:
            ...
            twitter_status_label.configure(text='Success! Found Twitter user',fg='green')
            ...
            twitter_status_label.configure(text='An unknown error occured, please try again.',fg='red')
...
def runDetector():
    ...
    tweets.extend(Cursor(client.twitter_client.user_timeline, 
                            id=user.id,tweet_mode='extended',
                            wait_on_rate_limit=True).items(tweet_amt))
...
#APP TITLE LABEL
title_label = Label(root,text=TITLE,bg=TITLE_BG,
                    anchor=CENTER,font=TITLEFONT)

title_label.grid(row=0,column=1,sticky='nsew',)

#CREDIT HEADER LABEL
header_label = Label(root,text='Developed by @ZenithO_o on Twitter',bg=BG,
                    anchor=N,font=HEADERFONT)
...
twitter_entry = Entry(root,width=40,bg=FIELD_BG,fg="gray30",bd=0,
                    justify=CENTER,font=DEFAULTFONT)
...
twitter_button = Button(root,text='Get User',padx=10,pady=5,bd=0,bg=BUTTON_BG,fg="snow",
                    font=DEFAULTFONT,justify=CENTER,command=testUser)

twitter_button.grid(row=5,column=1)

#TWITTER BUTTON STATUS LABEL
twitter_status_label = Label(root,text='',bg=BG,fg="red",
                    anchor=N,justify=CENTER,font=DISCLAIMERFONT)

twitter_status_label.grid(row=6,column=1)

#TWEET NUM SCALE
tweet_scale = Scale(root,font=DEFAULTFONT,label="Number of tweets to scrape:", from_=0,to=tweet_limit,relief=FLAT,
                    bg=BG,bd=0,orient=HORIZONTAL,sliderlength=20,sliderrelief=GROOVE,highlightthickness=0,length=300,fg="gray20",troughcolor=FIELD_BG,
                    tickinterval=tick_interval,state=DISABLED,command=updateAmt)

tweet_scale.grid(row=7,column=1)

#RUN DETECTOR BUTTON
run_detector_button = Button(root,text='Run detector',bg=BUTTON_DISABLED_BG,fg="snow",font=DEFAULTFONT,
                    justify=CENTER,padx=10,pady=5,bd=0,state=DISABLED,command=runDetector)

run_detector_button.grid(row=8,column=1)

#RUN DETECTOR BUTTON STATUS LABEL
run_detector_status_label = Label(root,text='',bg=BG,
                    anchor=N,justify=CENTER,font=DISCLAIMERFONT)

run_detector_status_label.grid(row=9,column=1)

#RESULT PERCENTAGE
percentage_label = Label(root,text="0.0%",bg=BG,
                    anchor=CENTER,fg="gray60",font=BIGFONT)
percentage_label.grid(row=10,column=1)

#RESULT DESCRIPTION
description_label = Label(root,text="Input a user to get started",bg=BG,
                    anchor=CENTER,fg="gray60",font=HEADERFONT)
description_label.grid(row=11,column=1)

#ERROR MESSAGES
error_label = Label(root,text=keras_model_error_message+twitter_api_error_message,bg=BG,fg="red",
                    anchor=S,pady=30,justify=CENTER,font=DISCLAIMERFONT)
